...
import HeroImageCenterDark from '@public/images/fgce-hero.png';
    ...
    <div className="relative z-0 ml-auto h-[650px] w-full max-w-[724px]">
      <RevealAnimation delay={0.1} offset={100} useSpring={true} duration={2} direction="up">
        <figure className="absolute left-[40px] z-1 w-[200px] rotate-[8deg] max-sm:top-[10px] sm:left-0 md:top-5 lg:top-0 lg:w-[273px]">
          <Image src={HeroImageTop} alt="FGCE Alumni" className="w-full rounded-[20px] dark:hidden" />
          <Image src={HeroImageTopDark} alt="FGCE Alumni" className="hidden w-full rounded-[20px] dark:block" />
        </figure>
      </RevealAnimation>
      <RevealAnimation delay={0.2} offset={100} useSpring={true} duration={2}>
        <figure className="absolute top-[100px] left-0 -z-1 w-[450px] max-w-full md:left-[10px] lg:left-[50px] lg:w-[480px] xl:left-[80px] xl:w-[520px]">
          <Image
            src={HeroImageCenter}
            alt="FGCEOSA Hero"
            className="w-full rounded-[30px] border-4 border-white shadow-2xl dark:border-background-7"
          />
        </figure>
      </RevealAnimation>
      <RevealAnimation delay={0.3} offset={100} useSpring={true} duration={2} direction="right">
        <figure className="md:[250px] lp:right-[-20px] shadow-2 absolute top-[250px] right-0 z-1 w-[150px] max-w-full rotate-[-15deg] overflow-hidden rounded-[20px] xl:right-[25px] xl:w-[280px]">
          <Image src={HeroImageRight} alt="Community" className="w-full dark:hidden" />
          <Image src={HeroImageRightDark} alt="Community" className="hidden w-full dark:block" />
        </figure>
      </RevealAnimation>
      <RevealAnimation delay={0.4} offset={100} useSpring={true} duration={2} direction="left">
        <figure className="absolute bottom-[-20px] left-[2px] z-1 w-[600px] max-w-full sm:-bottom-[60px] sm:-left-[45px] md:w-[357px]">
          <Image src={HeroImageBottom} alt="Networking" className="w-full rounded-[20px] dark:hidden" />
          <Image src={HeroImageBottomDark} alt="Networking" className="hidden w-full rounded-[20px] dark:block" />
        </figure>
      </RevealAnimation>
    </div>
  );
